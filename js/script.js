...
const headerIntersectionObserver = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    ...
    Constants.ROOT.classList.toggle("scrolled", !isIntersecting);
  });
});
...
const headerResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    ...
    Constants.ROOT.style.setProperty("--header-wrapper-block-size", headerWrapperBlockSize);
  });
});
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/about.js


const aboutSlider = new core(".about-slider", {
  modules: [Navigation, Pagination, Keyboard, Autoplay,],
  navigation: {
    prevEl: ".about-slider__arrows .slider-arrows__arrow--prev",
    nextEl: ".about-slider__arrows .slider-arrows__arrow--next",
  },
  pagination: {
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/services.js


const servicesSlider = new core(".services-slider", {
  modules: [Navigation, Pagination, Keyboard,],
  navigation: {
    prevEl: ".services-slider__arrows .slider-arrows__arrow--prev",
    nextEl: ".services-slider__arrows .slider-arrows__arrow--next",
  },
  pagination: {
...
const reviewsSlider = new core(".reviews-slider", {
  modules: [Navigation, Pagination, Keyboard,],
  navigation: {
    prevEl: ".reviews-slider__arrows .slider-arrows__arrow--prev",
    nextEl: ".reviews-slider__arrows .slider-arrows__arrow--next",
  },
  pagination: {
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/nav-links.js
let navItemResizeObserver;
const navLinks = document.querySelectorAll(".header-nav__link");

navLinks?.forEach(navLink => {
  navLink.addEventListener("gotoactive", () => {
    const navItem = navLink.closest(".header-nav__item");

    if (navItem) {
      navItemResizeObserver?.disconnect();

      navItemResizeObserver = new ResizeObserver(entries => {
        entries.forEach(entry => {
          const { target, borderBoxSize } = entry;
          const { inlineSize } = borderBoxSize[0];
          const { left: targetLeft } = target.getBoundingClientRect();
          const navList = target.closest(".header-nav__list");

          if (navList) {
            const { left: navListLeft } = navList.getBoundingClientRect();
            const relativeLeft = targetLeft - navListLeft;

            navList.style.setProperty("--active-nav-link-left", `${relativeLeft}px`);
            navList.style.setProperty("--active-nav-link-width", `${inlineSize}px`);
          }
        });
      });

      navItemResizeObserver.observe(navItem);
    }
  });

  navLink.addEventListener("gotoinactive", () => {
    const navList = navLink.closest(".header-nav__list");

    navList?.style.removeProperty("--active-nav-link-width");
  });
});
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/more-form.js
/** @type {HTMLFormElement} */
const moreForm = document.querySelector(".more__form[data-test]");

moreForm?.addEventListener("submit", event => {
  event.preventDefault();

  const inner = moreForm.querySelector(".form__inner");
  const thank = moreForm.querySelector(".thank");
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/banner-animation.js
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/text-animations.js


document.addEventListener("DOMContentLoaded", () => {
  const textAnimationBlockIntersectionObserver = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
      const { target, isIntersecting } = entry;
      const counter = target.querySelector("[data-count]");
      ...
      if (counter) {
        let { countStart, countEnd, countSuffix } = counter.dataset;

        if (countStart && countEnd) {
          countStart = +countStart;
          countEnd = +countEnd

          const counterUp = new CountUp(counter, countEnd, {
            startVal: countStart,
            duration: 5,
            separator: " ",
            suffix: countSuffix ? ` ${countSuffix}` : "",
          });

          counterUp.start();
          counterUp.options.onCompleteCallback = () => {
            const waitingTextAnimation = document.querySelector(".text-animation--waiting");

            counterUp.reset();
            counter.innerHTML = `${countEnd.toLocaleString("ru")}${countSuffix ? ` ${countSuffix}` : ""}`;
            waitingTextAnimation?.classList.remove(".text-animation--waiting");
            waitingTextAnimation?.classList.add("animate");
          };
        }
      }
    });
  ...
  const textAnimationBlocks = document.querySelectorAll(".text-animation:not(.text-animation--waiting)");

  textAnimationBlocks?.forEach(textAnimationBlock => {
    textAnimationBlockIntersectionObserver.observe(textAnimationBlock);
  });
});

;// CONCATENATED MODULE: ./src/js/scripts/scripts.js
// import "./scripts/spollers.js";
...
class Goto {
  ...
  #highlight(block, link) {
    ...
    const activeEvent = new Event("gotoactive");
    const inactiveEvent = new Event("gotoinactive");
      ...
      entries.forEach(entry => {
        ...
        blockIntersectionObserver = new IntersectionObserver(entries => {
          entries.forEach(entry => {
            ...
            isIntersecting ? link.dispatchEvent(activeEvent) : link.dispatchEvent(inactiveEvent);
          });
...
const goto_goto = new Goto(".header-nav__link");
